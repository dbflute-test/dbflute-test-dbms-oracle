...
public class ResolaDBFluteInitializer {
     ...
     */
    public ResolaDBFluteInitializer(javax.sql.DataSource dataSource) {
        if (dataSource == null) {
    ...
    protected void setupDataSourceHandler(String dataSourceFqcn) { // for Spring
        final ResolaDBFluteConfig config = ResolaDBFluteConfig.getInstance();
        ...
        if (needsSpringTransactionalDataSource(dataSourceFqcn)) {
            config.unlock();
            config.setDataSourceHandler(new ResolaDBFluteConfig.SpringTransactionalDataSourceHandler());
        }
    }
